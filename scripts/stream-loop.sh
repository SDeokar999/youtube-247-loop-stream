...
# makes a single job resilient for its ~6 hour lifetime. The GitHub Actions
# workflow re-runs this script fresh every 6 hours (see stream.yml).
set -uo pipefail

STREAM_URL="rtmp://a.rtmp.youtube.com/live2/${YT_STREAM_KEY}"
VIDEO_PATH="${VIDEO_PATH:-video/loop.mp4}"
BUDGET="${JOB_BUDGET_SECONDS:-21000}"

deadline=$((SECONDS + BUDGET))

while [ "$SECONDS" -lt "$deadline" ]; do
  echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] starting ffmpeg"
